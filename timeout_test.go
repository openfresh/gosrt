...
func TestAcceptTimeoutMustReturn(t *testing.T) {
	...
	ch := make(chan error)
	go func() {
		if err := ln.(*SRTListener).SetDeadline(noDeadline); err != nil {
			t.Error(err)
		}
		if err := ln.(*SRTListener).SetDeadline(time.Now().Add(10 * time.Millisecond)); err != nil {
			t.Error(err)
		}
		c, err := ln.Accept()
		if err == nil {
...
func TestAcceptTimeoutMustNotReturn(t *testing.T) {
	...
	ch := make(chan error)
	go func() {
		if err := ln.(*SRTListener).SetDeadline(time.Now().Add(-5 * time.Second)); err != nil {
			t.Error(err)
		}
		if err := ln.(*SRTListener).SetDeadline(noDeadline); err != nil {
			t.Error(err)
...
func testVariousDeadlines(t *testing.T) {
	...
	for _, timeout := range []time.Duration{
		...
		for run := 0; run < numRuns; run++ {
			...
			select {
			case res := <-actvch:
				if nerr, ok := res.err.(net.Error); ok && nerr.Timeout() {
